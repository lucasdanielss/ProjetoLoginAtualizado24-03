...
from tkinter import *
from AppUsuarios import Usuarios  # Certifique-se de que a classe Usuarios está no módulo AppUsuarios
...
class SistemaGestao:
    ...
    def buscarUsuario(self):
        user = Usuarios()  # Certifique-se de que a classe Usuarios está corretamente definida
        idusuario = self.idUsuario.get()
        user_data = user.selectUser(idusuario)
        if user_data:
            self.idUsuario.delete(0, END)
            self.idUsuario.insert(INSERT, user_data['idusuario'])
            self.nome.delete(0, END)
            self.nome.insert(INSERT, user_data['nome'])
            self.telefone.delete(0, END)
            self.telefone.insert(INSERT, user_data['telefone'])
            self.email.delete(0, END)
            self.email.insert(INSERT, user_data['email'])
            self.usuario.delete(0, END)
            self.usuario.insert(INSERT, user_data['usuario'])
            self.senha.delete(0, END)
            self.senha.insert(INSERT, user_data['senha'])
        else:
